...
//!
//! Re-exports the `phenotype-voxel` kernel verbatim. Consumer adapters
//! that need ECS/engine integration live in thin binding crates that
//! import from here.
//!
//! `material_pbr` contains the PBR material policy substrate, folded from
//! `civis-platform-wt/crates/voxel/src/material_pbr.rs`.
//! Pure Rust, engine-agnostic: no Bevy, no wgpu, no asset loader.
//! Only dependency beyond `serde` is `phenotype_voxel::MaterialId`.

// Re-export the full kernel surface for convenience.
